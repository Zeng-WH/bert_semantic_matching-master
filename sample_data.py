...
def create_positive(train_seed, df):
    ...
    for item in train_seed:
        ...
        sentence2 = str(df.iloc[item, temp_seed[1]]).strip()
        label = str(1).strip()
...
def create_negative(train_seed_neg_1, train_seed_neg_2, df):
    ...
    for item1, item2 in zip(train_seed_neg_1, train_seed_neg_2):
        ...
        temp_seed_2 = np.arange(9)
        np.random.shuffle(temp_seed_1)
        np.random.shuffle(temp_seed_2)
        sentence1 = str(df.iloc[item1, temp_seed_1[0]]).strip()
        sentence2 = str(df.iloc[item2, temp_seed_2[0]]).strip()
        label = str(0).strip()
        temp_dict['sent_1'] = sentence1
        temp_dict['sent_2'] = sentence2
        temp_dict['label'] = label
...
def main():
    np.random.seed(42)
    df = read_csv('/home1/CM2021/zwh/QQdata/data_sample.csv')
    ...
    with open('/home1/CM2021/zwh/QQdata/1-1/train_data.json', 'w') as w:
        json.dump(train_list_save, w)
    with open('/home1/CM2021/zwh/QQdata/1-1/dev_data.json', 'w') as w:
        json.dump(dev_list_save, w)
    with open('/home1/CM2021/zwh/QQdata/1-1/test_data.json', 'w') as w:
